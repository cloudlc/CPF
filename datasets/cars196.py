...
def Give(opt, datapath):
    ...
    #         im_paths.append(os.path.join(root, im_path))
    #         ys.append(y)
    #         I += [index]
    #         index += 1
    ...
    image_dict    = {}
    for img_path, key in zip(im_paths, ys):
    # for key, img_path in image_list:
        if not key in image_dict.keys():
            image_dict[key] = []
        image_dict[key].append(os.path.join(root, img_path))
    ...
    if opt.use_tv_split:
        ...
        val_conversion = {i:key for i,key in enumerate(val)}
        ###
    ...
    ###
    train_conversion = {i:key for i,key in enumerate(train)}
    test_conversion  = {i:key for i,key in enumerate(test)}
