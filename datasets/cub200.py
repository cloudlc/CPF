...
def Give(opt, datapath):
    image_sourcepath  = datapath+'/train'
    image_testpath  = datapath+'/test'
    ...
    for key in image_classes:
        for x in os.listdir(image_testpath+'/'+key):
            if '._' not in x:
                image_list[int(key.split('.')[0])-1].append(image_testpath+'/'+key+'/'+x)
