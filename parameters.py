...
def basic_training_parameters(parser):
    ...
    parser.add_argument('--arch',             default='resnet34_frozen_normalize',  type=str,   help='Underlying network architecture. Frozen denotes that \
...
def loss_specific_parameters(parser):
    ...
    ### CPF
    parser.add_argument('--loss_cpf_tau', default=0.2, type=float, help='hard threshold to eliminate uninformative negative elements.')   
    parser.add_argument('--loss_cpf_psi',  default=0.2, type=float, help='Offset term for controlling the magnitude of negative loss items.') 
    parser.add_argument('--loss_cpf_sp',  default=1.0,      type=float, help='Scaling hyperparameter for negative pair weighting.')
    parser.add_argument('--loss_cpf_sn', default=1.0,     type=float, help='Scaling hyperparameter for negative pair weighting.')
    parser.add_argument('--loss_cpf_mu', default=0.0, type=float, help='Bias term at which the negative weighting function is of value 1.')    
    parser.add_argument('--loss_cpf_b',  default=2,      type=float, help='Constant bias term added to both numerator and denominator in balancing formula.')
